...
from ultralytics.utils.metrics import OKS_SIGMA, bbox_Inner_iou
...
class BboxLoss(nn.Module):
    ...
    def forward(self, pred_dist, pred_bboxes, anchor_points, target_bboxes, target_scores, target_scores_sum, fg_mask):
        ...
        iou = bbox_iou(pred_bboxes[fg_mask], target_bboxes[fg_mask], xywh=False, CIoU=True)  # 使用CIoU
...
class BboxInnerLoss(nn.Module):
    ...
    def forward(self, pred_dist, pred_bboxes, anchor_points, target_bboxes, target_scores, target_scores_sum, fg_mask):
        """
        IoU loss.
        其中，如果iou是一个元组，则说明使用了Weighted IoU（WIoU）进行计算，
            如果iou是一个元组且长度为2，则说明使用了WIoU，
                需要将两个元素分别相乘再相减，最后再乘上权重weight,求和后再除以目标得分之和target_scores_sum。
            如果iou是一个元组但长度不为2，则说明使用了普通的WIoU，
                需要将两个元素相乘再乘上权重weight,求和后再除以目标得分之和target_scores_sum。
        如果iou不是元组，则说明使用了普通的IoU，需要将1.0减去iou再乘上权重weight，最后再除以目标得分之和target_scores_sum。
        """
        weight = target_scores.sum(-1)[fg_mask].unsqueeze(-1)
        iou = bbox_iou(pred_bboxes[fg_mask], target_bboxes[fg_mask], xywh=False, CIoU=True)  # 使用CIoU
        # loss_iou = ((1.0 - iou) * weight).sum() / target_scores_sum
        # # alpha=1，ratio=1时为普通使用IoU
        # iou = bbox_Inner_iou(pred_bboxes[fg_mask], target_bboxes[fg_mask], xywh=False, CIoU=True)  # ratio=0.7-0.8 提升大目标
        # obj_sz = (tobj.size()[2], tobj.size()[3])  # feat_sz 未改
...
class v8DetectionLoss:
    ...
    def __init__(self, model):  # model must be de-paralleled
        ...
        self.bbox_loss = BboxInnerLoss(m.reg_max - 1, use_dfl=self.use_dfl).to(device)
...
class v8SegmentationLoss(v8DetectionLoss):  # 计算loss
