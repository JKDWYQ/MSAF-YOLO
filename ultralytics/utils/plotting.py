...
class Annotator:
    ...
    def masks(self, masks, colors, im_gpu, alpha=0.5, retina_masks=False):
        ...
        self.im[:, :, :3] = im_mask_np if retina_masks else scale_image(im_mask_np, im_mask_np.shape)  # 4ch->3ch
        if self.pil:
            # Convert im back to PIL and update draw
            self.fromarray(self.im[:, :, :3])
...
def plot_labels(boxes, cls, names=(), save_dir=Path(''), on_plot=None):
    ...
    # Plot dataset labels
    LOGGER.info(f"Plotting labels to {save_dir / 'labels.png'}... ")  # 4ch
    ...
    plt.savefig(save_dir / 'labels_correlogram.png', dpi=200)  # 4ch
    plt.close()
...
def save_one_box(xyxy, im, file=Path('im.png'), gain=1.02, pad=10, square=False, BGR=False, save=True):
    ...
    if save:
        ...
        f = str(increment_path(file).with_suffix('.png'))  # 4ch
