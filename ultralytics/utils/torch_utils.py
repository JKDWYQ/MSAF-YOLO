...
def torch_distributed_zero_first(local_rank: int):
    ...
    Decorator使分布式训练中的所有进程等待每个local_master做一些事情。"""
...
def get_flops(model, imgsz=640):
    ...
    try:
        ...
        im = torch.empty((1, p.shape[1], stride, stride), device=p.device)  # input image in BCHW format todo 4->p.shape[1]
...
def make_divisible(x, divisor):
    """Returns nearest x divisible by divisor.返回最近的可被除数整除的x"""
