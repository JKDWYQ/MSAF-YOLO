...
def on_train_epoch_end(trainer):
    ...
    if trainer.epoch == 1:
        _log_images({f.stem: str(f) for f in trainer.save_dir.glob('train_batch*.png')}, 'Mosaic')  # 4ch
...
def on_val_end(validator):
    ...
    if run:
        ...
        _log_images({f.stem: str(f) for f in validator.save_dir.glob('val*.png')}, 'Validation')  # 4ch
